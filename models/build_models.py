...
from .vit import DropPath
from .vit import ViT as vision_transformer
...
class DNM(nn.Module):
    def __init__(self, in_channel, out_channel, num_branch=5, synapse_activation=nn.Sigmoid,
                 dendritic_activation=nn.Sigmoid, soma=nn.Softmax, **kwargs):
...
class DVT(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        if config['mlp_head'] == 'original':
            ...
            self.mlp_head = DNM(**dnm_config)

        self.net = ViT(**vit_config, mlp_head=self.mlp_head)
...
class OriViT(nn.Module):
    def __init__(self, config, device='cpu'):
        ...
        self.net = vision_transformer(**vit_config, mlp_head=self.mlp_head).to(device)
...
@register_model
def DVT_vit_base_patch14_224(num_classes=1000, pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None,
                             **kwargs):
    config = {
        "ViT": {
            "img_size": 224,
    ...
    net = DVT(config, **kwargs)
    return net



@register_model
def DVT_vit_base_patch16_224(num_classes=1000, pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None,
                             **kwargs):
    config = {
        "ViT": {
            "img_size": 224,
...
@register_model
def DVT_vit_base_patch32_224(num_classes=1000, pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None,
                             **kwargs):
    config = {
        "ViT": {
            "img_size": 224,
...
@register_model
def DVT_vit_large_patch16_224(num_classes=1000, pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None,
                             **kwargs):
    config = {
        "ViT": {
            "img_size": 224,
...
@register_model
def DVT_vit_large_patch32_224(num_classes=1000, pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None,
                             **kwargs):
    config = {
        "ViT": {
            "img_size": 224,
...
@register_model
def DVT_vit_huge_patch14_224(num_classes=1000, pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None,
                             **kwargs):
    config = {
        "ViT": {
            "img_size": 224,
...
#     net = create_model('DVT_vit_base_patch16_224', num_classes=5)
#     input_size = torch.randn(1, 3, 224, 224)
#     output = net(input_size)
#     print(output.size()) # torch.Size([1, 5])
